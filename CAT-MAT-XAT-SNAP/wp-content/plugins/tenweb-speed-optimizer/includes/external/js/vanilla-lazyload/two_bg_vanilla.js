...
function two_replace_backgrounds(elementor_elements = false) {
  ...
  if (!elementor_elements) {
    two_elements_list = document.querySelectorAll(
      "*:not(br):not(hr):not(iframe):not(pre)"
    );
  } else {
    two_elements_list = elementor_elements.querySelectorAll(
  ...
  two_elements_list.forEach((elem) => {
    ...
    if (
      bg_image === "none" ||
      bg_image.indexOf(window["two_svg_placeholder"]) === -1
    ) {
      return;
    }
  ...
  if (typeof two_lazyLoadInstance === "undefined") {
    window.two_lazyLoadInstance = new LazyLoad({
      callback_applied: function (element, instance) {
        ...
        var bgDataValue = element.getAttribute(
          "data-" + settings.data_bg_multi
        );
        if (!bgDataValue) {
          return;
        }
        ...
        if (
          window
            ...
            .getPropertyValue("background-image") !== bgDataValue
        ) {
